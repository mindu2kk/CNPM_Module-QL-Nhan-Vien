 ...
 * Giữ đúng cấu trúc diagram: JFrame + ActionListener
 */
public class ManageAccountFrm extends JFrame implements ActionListener {
    ...
    private void doEdit() {
        ...
        p.add(fUser);
        p.add(new JLabel("Mật khẩu mới (trống = giữ nguyên):"));
        p.add(fPass);
        p.add(new JLabel("Vai trò (phân quyền):"));
        p.add(fRole);
    ...
    private void doDelete() {
        ...
        AccountDAO dao = new AccountDAO();
        if (dao.deleteAccount(sel.getIdAcc())) {
