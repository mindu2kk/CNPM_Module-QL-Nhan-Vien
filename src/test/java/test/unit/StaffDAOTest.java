...
public class StaffDAOTest {
    ...
    // Tên dùng trong các test add/update/delete để tránh xung đột dữ liệu thật
    ...
    /** Thêm nhân viên mới hợp lệ → trả về true */
    @Test
    public void test4_AddStaff_Success() {
        // Dọn dữ liệu cũ nếu còn sót từ lần chạy trước
        cleanupTestUser();

        Staff s = new Staff(0, TEST_NAME, "Librarian", "0900000000", "test@lib.vn");
        assertTrue("Thêm nhân viên hợp lệ phải trả về true", dao.addStaff(s));
    }
    ...
    public void test6_UpdateStaff_Success() {
        ...
        assertEquals("SĐT phải được cập nhật", "0911111111", updated[0].getTel());
    }
    ...
    /** Dọn dữ liệu test cũ để tránh ảnh hưởng giữa các lần chạy */
    private void cleanupTestUser() {
        Staff[] old = dao.searchStaff(TEST_NAME);
        for (Staff s : old) {
            dao.deleteStaff(s.getId());
        }
    }
}
